...
DECLARE
    ...
    url varchar2(1000) := 'http://int2-ecm.lexisnexis.com/RealTimeOutbound/retention/publishEngagementRecords?contactLogin=patelgv&contactEmail=gaurang.patel@lexisnexis.com&engagementSource=GCRM';
...
BEGIN
      ...
      --  content := 'Welcome';
       ...
       dbms_lob.createtemporary(content_blob, TRUE);
        ...
        loop
         dbms_lob.append(content_blob,  UTL_RAW.CAST_TO_RAW( '"'||C1.AFF_PER_ID||'","'||C1.PREFIX||'","'||C1.FIRST_NAME||'","'||C1.MIDDLE_NAME||'","'||C1.LAST_NAME||'","'||C1.EMAIL||'","'||C1.ADDRESS_LINE1||'","'||C1.ADDRESS_LINE2||'","'||C1.City||'","'||C1.State||'","'||C1.Postal_Code||'","'||C1.Country||'","'||C1.ENGAGEMENT_DATE||'","'||C1.ENGAGEMENT_TYPE||'","'||C1.CUSTOMER_FLAG||'"'|| chr(13) || chr(10)));
         i := i + 1; 
      end loop;
